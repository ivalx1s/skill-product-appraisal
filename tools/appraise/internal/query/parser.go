// Package query implements the mini-query DSL parser for the appraise CLI.
//
// Grammar:
//   batch     = query (";" query)*
...
const (
	tokenIdent     tokenType = iota // identifier: letters, digits, underscores, hyphens, dots
	tokenLParen                     // (
	tokenRParen                     // )
	tokenLBrace                     // {
	tokenRBrace                     // }
...
// Query is the top-level node containing one or more statements.
type Query struct {
...
type Statement struct {
	Operation string   // e.g. "calc", "list", "schema"
...
var validOperations = map[string]bool{
	"calc":    true, // calc(module.function, param=value) { fields }
	"list":    true, // list(modules) or list(functions, module=pricing)
	"schema":  true, // schema(module) or schema()
	"summary": true, // summary() — overview of all modules and functions
}
...
func (p *parser) parseStatement() (*Statement, error) {
	...
	if !validOperations[opTok.val] {
		return nil, fmt.Errorf("unknown operation %q at position %d; valid: calc, list, schema, summary", opTok.val, opTok.pos)
	}
	...
	if p.peek().typ == tokenLBrace {
		fieldNames, err = p.parseProjection()
		if err != nil {
			return nil, err
		}
	}

	return &Statement{Operation: opTok.val, Args: args, Fields: fieldNames}, nil
}
...
func (p *parser) parseProjection() ([]string, error) {
	...
	for p.peek().typ != tokenRBrace && p.peek().typ != tokenEOF {
		...
		// Expand presets
		if expanded, ok := fields.Presets[tok.val]; ok {
			fieldNames = append(fieldNames, expanded...)
			continue
		}
		if !fields.ValidFields[tok.val] {
